/**
 * ORTA PETLER kategorisindeki tüm ürünlere 12'li kutu ayarlar.
 * pack_size = 12, pack_label_tr = Kutu, pack_label_en = Box, pack_label_fr = Boîte
 *
 * Çalıştırma: npx tsx scripts/set-orta-petler-12-pack.ts
 */
import { db } from '../src/db';
import { categories, products } from '../src/db/schema';
import { eq } from 'drizzle-orm';

const CATEGORY_NAMES = ['ORTA PETLER', 'Orta Petler', 'orta petler'];

async function run() {
  console.log('ORTA PETLER kategorisi aranıyor...\n');

  const allCategories = await db.select().from(categories);
  const ortaCategory = allCategories.find(
    (c) => CATEGORY_NAMES.includes(c.name) || c.name.toUpperCase().replace(/İ/g, 'I') === 'ORTA PETLER'
  );
