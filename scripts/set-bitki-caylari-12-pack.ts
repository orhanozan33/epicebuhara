/**
 * BİTKİ ÇAYLARI kategorisindeki tüm ürünlere 12'li kutu ayarlar.
 * pack_size = 12, pack_label_tr = Kutu, pack_label_en = Box, pack_label_fr = Boîte
 *
 * Çalıştırma: npx tsx scripts/set-bitki-caylari-12-pack.ts
 */
import { db } from '../src/db';
import { categories, products } from '../src/db/schema';
import { eq } from 'drizzle-orm';

const CATEGORY_NAMES = ['BİTKİ ÇAYLARI', 'Bitki Çayları', 'bitki çayları', 'BITKI CAYLARI'];
...
async function run() {
  console.log('BİTKİ ÇAYLARI kategorisi aranıyor...\n');

  const allCategories = await db.select().from(categories);
  const category = allCategories.find(
    (c) => CATEGORY_NAMES.includes(c.name) || normalize(c.name) === 'BITKI CAYLARI'
  );
